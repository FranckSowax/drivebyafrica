...
export function QuotePDFModal({ isOpen, onClose, quoteData, user, profile, defaultQuoteNumber }: QuotePDFModalProps) {
  ...
  const handlePaymentSuccess = async ({ externalReference, paymentMethod }: { externalReference: string; paymentMethod: string }) => {
    ...
    try {
      ...
      const orderRes = await fetch('/api/orders/from-quote', {
        method: 'POST',
        body: JSON.stringify({
                ...
                <div className="p-4 sm:p-6 md:p-12 space-y-6 md:space-y-8">
                        ...
                        <tbody className="divide-y divide-gray-100 text-gray-700">
                          <tr>
                            <td className="px-3 py-3 md:px-6 md:py-4">
                              ...
                              {quoteData.calculations.hasExportTax && (
                                <span className="ml-1 md:ml-2 text-[10px] text-gray-400">(inclut taxe export)</span>
                              )}
                            </td>
                            <td className="px-3 py-3 md:px-6 md:py-4 text-right font-mono font-medium whitespace-nowrap">{formatCurrency(quoteData.calculations.vehiclePrice)}</td>
                          </tr>
                          <tr>
                            <td className="px-3 py-3 md:px-6 md:py-4">
                              <span className="flex items-center gap-1 md:gap-2 flex-wrap">
                                Transport maritime
                                <span className="text-[9px] md:text-[10px] px-1 md:px-1.5 py-0.5 bg-gray-100 rounded text-gray-500">{quoteData.shippingTypeName}</span>
                              </span>
                            </td>
                            <td className="px-3 py-3 md:px-6 md:py-4 text-right font-mono font-medium whitespace-nowrap">{formatCurrency(quoteData.calculations.shippingCost)}</td>
                          </tr>
                          <tr>
                            <td className="px-3 py-3 md:px-6 md:py-4">Assurance cargo (2.5%)</td>
                            <td className="px-3 py-3 md:px-6 md:py-4 text-right font-mono font-medium whitespace-nowrap">{formatCurrency(quoteData.calculations.insuranceCost)}</td>
                          </tr>
                          <tr>
                            <td className="px-3 py-3 md:px-6 md:py-4">Inspection & Documents</td>
                            <td className="px-3 py-3 md:px-6 md:py-4 text-right font-mono font-medium whitespace-nowrap">{formatCurrency(quoteData.calculations.inspectionFee)}</td>
                          </tr>
                          <tr className="bg-orange-50/50">
                            <td className="px-3 py-3 md:px-6 md:py-5">
                              ...
                              {quoteData.calculations.hasExportTax && (
                                <span className="text-[10px] md:text-xs text-gray-500 font-normal">Inclut taxe et douane export</span>
                              )}
                            </td>
                            <td className="px-3 py-3 md:px-6 md:py-5 text-right">
                              <span className="text-base md:text-xl font-black text-mandarin font-mono">{formatCurrency(quoteData.calculations.total)}</span>
                            </td>
                          </tr>
                        </tbody>
                      </table>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 pt-4">
                    <div className="bg-green-50 border border-green-100 p-3 md:p-5 rounded-xl">
                      ...
                      <p className="text-xl md:text-2xl font-black text-gray-900 mb-1">1 000 USD</p>
                      <p className="text-xs md:text-sm font-medium text-gray-600 mb-2">≈ 600 000 FCFA</p>
