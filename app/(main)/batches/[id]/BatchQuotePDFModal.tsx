...
interface BatchQuotePDFModalProps {
  isOpen: boolean;
  onClose: () => void;
  quoteData: BatchQuoteData | null;
...
export function BatchQuotePDFModal({ isOpen, onClose, quoteData, user, profile }: BatchQuotePDFModalProps) {
  ...
  useEffect(() => {
    if (isOpen && quoteData) {
      ...
      setQuoteNumber(`DBA-LOT-${timestamp}-${random}`);
      setQuoteSaved(false);
      setPdfBlob(null);
      setPdfUrl(null);
    }
  ...
  const saveQuoteToDatabase = useCallback(async (qNumber: string): Promise<boolean> => {
    ...
    try {
      // Fetch with cookie-based auth
      const response = await fetch('/api/quotes', {
        method: 'POST',
        body: JSON.stringify({
          quote_number: qNumber,
          user_id: user.id,
          batch_id: quoteData.batchId,
          vehicle_make: quoteData.batchMake,
          vehicle_model: quoteData.batchModel,
          vehicle_year: quoteData.batchYear,
          quantity: quoteData.quantity,
          vehicle_price_usd: quoteData.unitPriceUSD * quoteData.quantity,
          vehicle_source: quoteData.batchSource,
          ...
          // Always send USD values - API will convert to XAF for storage
          shipping_cost_usd: calc.shippingCostUSD ?? 0,
          insurance_cost_usd: calc.insuranceCostUSD ?? 0,
          inspection_fee_usd: calc.inspectionFeeUSD ?? 0,
          total_cost_usd: calc.totalUSD ?? 0,
  ...
  const generatePDF = useCallback(async () => {
    ...
    try {
      ...
      doc.setTextColor(darkGray);
      doc.setFont('helvetica', 'bold');
      doc.text("DEVIS D'IMPORTATION - LOT DE VEHICULES", margin, y);
      doc.setFontSize(9);
      doc.setTextColor(lightGray);
      doc.setFont('helvetica', 'normal');
      ...
      // Batch Box
      const batchX = margin + colWidth + 6;
      drawBox(batchX, y, colWidth, clientBoxHeight, surface);
      doc.setFontSize(9);
      doc.setTextColor(mandarin);
      doc.setFont('helvetica', 'bold');
      doc.text('LOT DE VEHICULES', batchX + 5, y + 7);
      doc.setFontSize(8);
      doc.setTextColor(lightGray);
      doc.setFont('helvetica', 'normal');
      doc.text('Marque / Modele', batchX + 5, y + 14);
      doc.setTextColor(darkGray);
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(10);
      doc.text(`${quoteData.batchMake} ${quoteData.batchModel}`, batchX + 5, y + 21);
      doc.setFontSize(8);
      doc.setTextColor(lightGray);
      doc.setFont('helvetica', 'normal');
      doc.text(`Annee: ${quoteData.batchYear}`, batchX + 5, y + 28);
      doc.text(`Quantite: ${quoteData.quantity} vehicules`, batchX + 5, y + 33);
      doc.text(`Origine: ${SOURCE_NAMES[quoteData.batchSource]}`, batchX + 5, y + 38);
      ...
      const costs = [
        { label: `Prix vehicules (FOB) - ${formatCurrency(quoteData.calculations.unitPrice)} x ${quoteData.quantity}`, value: quoteData.calculations.vehiclePrice },
        { label: `Transport maritime - ${quoteData.shippingTypeName} x ${quoteData.quantity}`, value: quoteData.calculations.shippingCost },
        { label: 'Assurance cargo (2.5%)', value: quoteData.calculations.insuranceCost },
        { label: `Inspection & Documents x ${quoteData.quantity}`, value: quoteData.calculations.inspectionFee },
      ];
      ...
      // Deposit box
      const depositPerVehicle = 1000;
      const totalDeposit = depositPerVehicle * quoteData.quantity;
      drawBox(margin, y, contentWidth, 32, '#ECFDF5', jewel);
      doc.setFontSize(10);
      doc.setTextColor(jewel);
      doc.setFont('helvetica', 'bold');
      doc.text('ACOMPTE REQUIS POUR BLOQUER LE LOT', margin + 5, y + 8);
      doc.setFontSize(16);
      doc.setTextColor(darkGray);
      doc.text(`${totalDeposit.toLocaleString()} USD`, margin + 5, y + 18);
      doc.setFontSize(10);
      doc.setTextColor(mediumGray);
      doc.setFont('helvetica', 'normal');
      doc.text(`(${depositPerVehicle.toLocaleString()} USD x ${quoteData.quantity} vehicules)`, margin + 50, y + 18);
      doc.setFontSize(8);
      doc.setTextColor(jewel);
      doc.text("Cet acompte declenche le rapport d'inspection detaille des vehicules", margin + 5, y + 26);
      y += 38;
      ...
      const steps = [
        { num: '1', text: `Versez l'acompte de ${totalDeposit.toLocaleString()}$ pour bloquer le lot` },
      ...
      const noteText = `* Ce devis est une estimation pour ${quoteData.quantity} vehicules. Les frais de dedouanement ne sont pas inclus et varient selon la reglementation de ${quoteData.destination.country}. Devis valable 7 jours.`;
  ...
  }, [quoteData, user, quoteNumber, isGenerating, formatCurrency, toast]);
  ...
  const handleShare = async () => {
    if (pdfBlob) {
      try {
        const fileName = `Devis-Lot-${quoteNumber}.pdf`;
        const file = new File([pdfBlob], fileName, { type: 'application/pdf' });

        if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({
            title: `Devis Lot Driveby Africa ${quoteNumber}`,
            text: `Voici votre devis d'importation pour ${quoteData?.quantity} ${quoteData?.batchMake} ${quoteData?.batchModel}`,
            files: [file],
          });
        } else {
        ...
        if ((error as Error).name !== 'AbortError') {
          toast.error('Erreur lors du partage. Telechargement en cours...');
          const link = document.createElement('a');
          link.href = pdfUrl || '';
          link.download = `Devis-Lot-${quoteNumber}.pdf`;
          link.click();
        }
      }
    }
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
              <div>
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  <Boxes className="w-5 h-5 text-royal-blue" />
                  Devis Lot de Vehicules
                </h2>
              ...
              <div className="p-4 md:p-8 flex justify-center">
                {isGenerating ? (
                  <div className="flex flex-col items-center justify-center min-h-[400px]">
                    ...
                    <p className="text-gray-500 font-medium">Preparation de votre devis...</p>
                  </div>
                ) : quoteData ? (
                    ...
                    <div className="p-8 md:p-12 space-y-8">
                      <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                        ...
                        <div className="bg-orange-50 border border-orange-100 p-4 rounded-lg min-w-[200px]">
                          <p className="text-[10px] uppercase tracking-wider text-orange-600 font-bold mb-1">Devis Lot</p>
                          <p className="text-lg font-mono font-bold text-gray-900">{quoteNumber}</p>
                          <div className="flex items-center gap-2 mt-2 text-xs text-gray-500">
                            <Calendar className="w-3 h-3" />
                            <span>Emis le: {new Date().toLocaleDateString('fr-FR')}</span>
                          </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-gray-100">
                        ...
                        <div className="space-y-4">
                          <h4 className="text-xs uppercase tracking-widest text-gray-400 font-bold">Details du Lot</h4>
                          <div className="space-y-1">
                            <p className="text-sm font-medium text-gray-500">Vehicules</p>
                            <p className="text-base font-bold text-gray-900">{quoteData.batchMake} {quoteData.batchModel} ({quoteData.batchYear})</p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-sm px-2 py-0.5 bg-royal-blue/10 rounded text-royal-blue font-medium">
                                {quoteData.quantity} vehicules
                              </span>
                              <span className="text-sm px-2 py-0.5 bg-gray-100 rounded text-gray-600">
                                Origine: {SOURCE_NAMES[quoteData.batchSource]} {SOURCE_FLAGS[quoteData.batchSource]}
                              </span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="bg-blue-50 border border-blue-100 p-6 rounded-xl flex flex-col md:flex-row justify-between items-center gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-bold text-blue-900">Expedition Internationale</p>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <h4 className="text-xs uppercase tracking-widest text-gray-400 font-bold">Detail des Couts</h4>
                            ...
                            <tbody className="divide-y divide-gray-100 text-gray-700">
                              <tr>
                                <td className="px-6 py-4">
                                  ...
                                  <span className="ml-2 text-xs text-gray-400">
                                    ({formatCurrency(quoteData.calculations.unitPrice)} x {quoteData.quantity})
                                  </span>
                              ...
                              <tr>
                                <td className="px-6 py-4 flex items-center gap-2">
                                  Transport maritime
                                  <span className="text-[10px] px-1.5 py-0.5 bg-gray-100 rounded text-gray-500">{quoteData.shippingTypeName} x {quoteData.quantity}</span>
                                </td>
                              ...
                              <tr>
                                <td className="px-6 py-4">Inspection & Documents x {quoteData.quantity}</td>
                                <td className="px-6 py-4 text-right font-mono font-medium">{formatCurrency(quoteData.calculations.inspectionFee)}</td>
                              </tr>
                              <tr className="bg-orange-50/50">
                                <td className="px-6 py-5">
                                  <span className="font-black text-gray-900 uppercase tracking-tighter block">Total estime</span>
                                  {quoteData.calculations.hasExportTax && (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                        <div className="bg-green-50 border border-green-100 p-5 rounded-xl">
                          ...
                          <p className="text-2xl font-black text-gray-900 mb-1">{(1000 * quoteData.quantity).toLocaleString()} USD</p>
                          <p className="text-sm font-medium text-gray-600 mb-2">1 000 USD x {quoteData.quantity} vehicules</p>
                          <p className="text-[10px] text-green-600 leading-relaxed">
                            Le versement de cet acompte declenche l'inspection physique detaillee des vehicules.
                          </p>
                        </div>
                        <div className="space-y-3">
                          <h5 className="text-[10px] font-black text-gray-400 uppercase">Prochaines Etapes</h5>
                          <div className="space-y-2">
                            {[
                              `Paiement de l'acompte de ${(1000 * quoteData.quantity).toLocaleString()}$`,
                              "Rapports d'inspection complets",
                              "Reglement du solde et expedition",
                              "Suivi et livraison a destination"
                            ].map((step, i) => (
                      ...
                      <div className="pt-8 border-t border-gray-100">
                        <p className="text-[9px] text-gray-400 italic leading-relaxed text-center">
                          * Ce devis est une estimation pour {quoteData.quantity} vehicules basee sur les tarifs actuels. Les frais de dedouanement ne sont pas inclus et varient selon la reglementation de {quoteData.destination.country}. Devis valable 7 jours a compter de la date d'emission.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col items-center justify-center min-h-[400px]">
                    <p className="text-gray-500">Erreur de chargement ou donnees manquantes</p>
                  </div>
                )}
              </div>
