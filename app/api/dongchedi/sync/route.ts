 ...
 * - maxPages: number (default: 5, for full sync)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const maxPages = Math.min(body.maxPages || 5, 20); // Limit to 20 pages max
    const sinceDays = body.sinceDays || 1;

    const stats = { added: 0, updated: 0, removed: 0, errors: 0, totalProcessed: 0 };
