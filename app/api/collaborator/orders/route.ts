...
import { getMissingDocuments, type UploadedStatusDocument } from '@/lib/order-documents-config';
...
const COUNTRY_TO_SOURCE: Record<string, string[]> = {
  ...
  all: ['china', 'che168', 'dongchedi', 'korea', 'encar', 'dubai', 'dubicars'],
};
...
export async function GET(request: Request) {
  try {
    ...
    // If assigned_country is 'all' or null, don't filter
    const allowedSources = assignedCountry && assignedCountry !== 'all'
      ? COUNTRY_TO_SOURCE[assignedCountry] || []
      : null;
...
export async function PUT(request: Request) {
  try {
    ...
    if (orderId) {
      ...
      // Enforce document upload before status transition
      const uploadedDocuments = Array.isArray(order.uploaded_documents)
        ? (order.uploaded_documents as unknown as UploadedStatusDocument[])
        : [];
      const missingDocs = getMissingDocuments(oldStatus, uploadedDocuments)
        .filter(doc => !doc.autoGenerated);

      if (missingDocs.length > 0) {
        const missingLabels = missingDocs.map(d => d.label).join(', ');
        return NextResponse.json(
          {
            error: `Documents manquants pour le statut actuel: ${missingLabels}. Veuillez les uploader avant de changer le statut.`,
            error_zh: `当前状态缺少文件: ${missingDocs.map(d => d.labelZh).join(', ')}。请在更改状态之前上传。`,
            missingDocuments: missingDocs.map(d => ({ id: d.id, label: d.label, labelZh: d.labelZh, type: d.type })),
          },
          { status: 400 }
        );
      }
      ...
      // Notify admins about the status change (bidirectional notifications)
      ...
      await notifyAdmins(supabaseAdmin, {
        ...
        title: `Order ${order.order_number || `ORD-${orderId.slice(0, 8)}`} updated by collaborator`,
        titleZh: `协作员更新了订单 ${order.order_number || `ORD-${orderId.slice(0, 8)}`}`,
        message: `Status changed from "${oldStatus}" to "${orderStatus}" by collaborator`,
      ...
      console.log('✅ Admin notified of status change');
    ...
    // Notify admins about the status change (bidirectional notifications - legacy)
    const vehicleInfo = `${quote.vehicle_make} ${quote.vehicle_model} ${quote.vehicle_year || ''}`;
    const customerName = quoteProfile?.full_name || 'Customer';
    const orderNumber = quote.quote_number?.replace('QT-', 'ORD-') || `ORD-${quoteId.slice(0, 8)}`;

    await notifyAdmins(supabaseAdmin, {
      type: 'order_status_updated',
      title: `Order ${orderNumber} updated by collaborator`,
      titleZh: `协作员更新了订单 ${orderNumber}`,
      message: `Status changed from "${oldStatus}" to "${orderStatus}" by collaborator`,
      messageZh: `协作员将状态从"${oldStatus}"更改为"${orderStatus}"`,
      data: {
        quoteId,
      ...
      priority: 'normal',
      actionUrl: `/admin/orders?quoteId=${quoteId}`,
      relatedEntityType: 'order',
      relatedEntityId: quoteId,
      excludeUserId: authCheck.user.id,
    });

    console.log('✅ Admin notified of status change (legacy)');
